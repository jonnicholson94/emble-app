...
const ResponsesHeader = ({ title, id }: Props) => {
    return (
        <header className="h-[50px] w-full bg-white border-b border-paleGrey flex items-center justify-start px-[20px]">
            <Link className="mr-[20px]" href={`/research/${id}`}>
                <img src="/close.svg" alt="A close button to return to previous screen" />
            </Link>
            <Link className="text-sm text-light text-border" href={`/research/${id}`}>{title}</Link>
